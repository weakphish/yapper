...
/// Trait describing the ability to parse a Markdown note into structured entities.
///
/// Each backend consumes the same Markdown documents but may use different
/// strategies (regex, tree-sitter, external services, etc.) to extract data. The
/// rest of the system talks to parser back-ends exclusively through this interface.
...
// Re-export the regex-backed parser as the default backend until another
// implementation (e.g. tree-sitter) is available.
pub use regex_backend::RegexMarkdownParser;

/// Regex-based Markdown parser backend.
mod regex_backend {
    use super::*;
    use chrono::Utc;
    ...
    #[derive(Default)]
    pub struct RegexMarkdownParser;

    impl RegexMarkdownParser {
    ...
    impl NoteParser for RegexMarkdownParser {
    ...
    static TIME_PREFIX_RE: Lazy<Regex> = Lazy::new(|| {
        Regex::new(
            r#"^\s*-\s*(?P<time>[0-9]{1,2}:[0-9]{2}(?:\s?(?:am|pm|AM|PM))?)\s+(?P<rest>.*)$"#,
        )
    ...
    mod tests {
        ...
        fn parses_multiline_tasks_and_logs() {
            let parser = RegexMarkdownParser::new();
            let content = r#"
# 2025-03-15
